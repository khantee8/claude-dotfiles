...
Private, invite-only exam trainer, **live at https://exam.nanoteofficial.me (v1.2.0)**. v1 subject: **PECB ISO/IEC 27001 Lead Implementer** (285 questions, 38 scenarios), architecture multi-subject from day one. Three modes: **study** (reveal the answer without guessing — for a first pass through the bank), **practice** (instant feedback + written explanation) and **exam simulation** (dark "exam hall" UI, timer, flag-for-review, resume, auto-submit). Access = request → admin approval → magic link; the gate is `src/app/(app)/layout.tsx`, **no middleware**.
