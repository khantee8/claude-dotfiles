...
Private, invite-only exam trainer, **live at https://exam.nanoteofficial.me (v1.1.0)**. v1 subject: **PECB ISO/IEC 27001 Lead Implementer** (285 questions, 38 scenarios), architecture multi-subject from day one. Two modes: **practice** (warm "calm study" UI, instant feedback + written explanation per answer) and **exam simulation** (dark "exam hall" UI, timer, flag-for-review, resume, auto-submit). Question order is seed-shuffled with scenario blocks kept contiguous; **choice order is deliberately not shuffled** because 83 explanations name options by letter. Access = request → admin approval → magic link; the gate is `src/app/(app)/layout.tsx`, **no middleware**.
